/*
  # Normalize profiles schema to fix stack depth issues

  1. New Tables
    - `user_preferences` - stores dietary restrictions, allergies, health conditions, food preferences
    - `saved_meal_plans` - stores meal plans and nutrition strategies
    - Simplified `profiles` table with only basic user info

  2. Changes
    - Remove complex JSON fields from profiles table
    - Create separate normalized tables for preferences
    - Add proper indexes and RLS policies

  3. Security
    - Enable RLS on all new tables
    - Add policies for authenticated users to manage their own data
*/

-- Create user_preferences table to store all preference data separately
CREATE TABLE IF NOT EXISTS user_preferences (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  preference_type text NOT NULL CHECK (preference_type IN ('dietary_restriction', 'allergy', 'health_condition', 'food_preference', 'regional_preference')),
  preference_value text NOT NULL,
  created_at timestamptz DEFAULT now()
);

-- Create saved_meal_plans table
CREATE TABLE IF NOT EXISTS saved_meal_plans (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  name text NOT NULL,
  plan_type text NOT NULL CHECK (plan_type IN ('weekly_meal_plan', 'nutrition_strategy')),
  plan_content text NOT NULL,
  parsed_data jsonb DEFAULT '{}',
  is_active boolean DEFAULT false,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);

-- Enable RLS
ALTER TABLE user_preferences ENABLE ROW LEVEL SECURITY;
ALTER TABLE saved_meal_plans ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for user_preferences
CREATE POLICY "Users can manage own preferences"
  ON user_preferences
  FOR ALL
  TO authenticated
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- Create RLS policies for saved_meal_plans
CREATE POLICY "Users can manage own meal plans"
  ON saved_meal_plans
  FOR ALL
  TO authenticated
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_user_preferences_user_type ON user_preferences(user_id, preference_type);
CREATE INDEX IF NOT EXISTS idx_saved_meal_plans_user_type ON saved_meal_plans(user_id, plan_type, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_saved_meal_plans_active ON saved_meal_plans(user_id, is_active) WHERE is_active = true;

-- Create unique constraint to prevent duplicate preferences
CREATE UNIQUE INDEX IF NOT EXISTS idx_user_preferences_unique ON user_preferences(user_id, preference_type, preference_value);

-- Add trigger to update updated_at timestamp
CREATE OR REPLACE FUNCTION update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = now();
    RETURN NEW;
END;
$$ language 'plpgsql';

CREATE TRIGGER update_saved_meal_plans_updated_at 
    BEFORE UPDATE ON saved_meal_plans 
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();

-- Migrate existing data if profiles table has complex JSON fields
DO $$
BEGIN
  -- Check if preferences column exists and migrate data
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'preferences') THEN
    -- Migrate dietary restrictions
    INSERT INTO user_preferences (user_id, preference_type, preference_value)
    SELECT 
      user_id, 
      'dietary_restriction', 
      unnest(COALESCE((preferences->>'dietary_restrictions')::text[], ARRAY[]::text[]))
    FROM profiles 
    WHERE preferences->>'dietary_restrictions' IS NOT NULL
    ON CONFLICT DO NOTHING;

    -- Migrate regional preferences
    INSERT INTO user_preferences (user_id, preference_type, preference_value)
    SELECT 
      user_id, 
      'regional_preference', 
      preferences->>'regional_preference'
    FROM profiles 
    WHERE preferences->>'regional_preference' IS NOT NULL AND preferences->>'regional_preference' != ''
    ON CONFLICT DO NOTHING;
  END IF;

  -- Migrate allergies if column exists
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'allergies') THEN
    INSERT INTO user_preferences (user_id, preference_type, preference_value)
    SELECT 
      user_id, 
      'allergy', 
      unnest(COALESCE(allergies, ARRAY[]::text[]))
    FROM profiles 
    WHERE allergies IS NOT NULL AND array_length(allergies, 1) > 0
    ON CONFLICT DO NOTHING;
  END IF;

  -- Migrate health conditions if column exists
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'health_conditions') THEN
    INSERT INTO user_preferences (user_id, preference_type, preference_value)
    SELECT 
      user_id, 
      'health_condition', 
      unnest(COALESCE(health_conditions, ARRAY[]::text[]))
    FROM profiles 
    WHERE health_conditions IS NOT NULL AND array_length(health_conditions, 1) > 0
    ON CONFLICT DO NOTHING;
  END IF;

  -- Migrate food preferences if column exists
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'food_preferences') THEN
    INSERT INTO user_preferences (user_id, preference_type, preference_value)
    SELECT 
      user_id, 
      'food_preference', 
      unnest(COALESCE(food_preferences, ARRAY[]::text[]))
    FROM profiles 
    WHERE food_preferences IS NOT NULL AND array_length(food_preferences, 1) > 0
    ON CONFLICT DO NOTHING;
  END IF;
END $$;